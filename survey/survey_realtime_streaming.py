...
class FinalWorker:
    ...
    def __init__(self, answer, quesexternalId, ans_val, instNumber, responseLabel, orgarr, createObj,sub_id):
        self.answer = answer
                    ...
                    if 'answers' in obSub.keys() :  
                        ...
                        for ans in answersArr:
                                ...
                                try:
                                    ...
                                    if (len(ansFn['options']) == 0) or (('options' not in ansFn.keys())):
                                        try:
                                            ...
                                            final_worker = FinalWorker(ansFn,ansFn['externalId'], ansFn['value'], instNumber, ansFn['value'], orgArr, creatingObj,sub_id)
                                            final_worker.run()
                                            ...
                                            try:
                                                if type(ansFn['value']) == str or type(ansFn['value']) == int:
                                                    if quesOpt['value'] == ansFn['value'] :
                                                        orgArr = orgCreator(obSub["userProfile"]["organisations"])
                                                        final_worker = FinalWorker(ansFn,ansFn['externalId'], ansFn['value'], instNumber, quesOpt['label'], orgArr, creatingObj,sub_id)
                                                        final_worker.run()
                                                elif type(ansFn['value']) == list:
                                                    for ansArr in ansFn['value']:
                                                        if quesOpt['value'] == ansArr:
                                                            orgArr = orgCreator(obSub["userProfile"]["organisations"])
                                                            final_worker = FinalWorker(ansFn,ansFn['externalId'], ansArr, instNumber, quesOpt['label'], orgArr, creatingObj,sub_id)
                                                            final_worker.run()
                                            except KeyError:
                                                pass
                                except KeyError:
                            ...
                            if (
                                ...
                                inst_cnt = ''
                                fetchingQuestiondetails(ans, inst_cnt,str(obSub['_id']))
                            elif ans['responseType'] == 'matrix' and len(ans['value']) > 0:
                                inst_cnt =0
                                for instances in ans['value']:
                                    inst_cnt = inst_cnt + 1
                                    for instance in instances.values():
                                        fetchingQuestiondetails(instance,inst_cnt,str(obSub['_id']))
            else:                        
...
def main_data_extraction(obSub):
    ...
    if submission_exits_in_meta == False : 
        ...
        surveySubQuestionsObj['createdBy'] = obSub['createdBy']
        surveySubQuestionsObj['completedDate'] = obSub.get('completedDate', '')
        # Check if 'isAPrivateProgram' key exists
        try:
            surveySubQuestionsObj['isAPrivateProgram'] = obSub['isAPrivateProgram']
        except KeyError:
        ...
        # Insert data to sl-survey-meta druid datasource , irrespective of status 
    ...
    # processing for sl-survey-status-inprogress datsource 
